...
export default function Sidebar() {
  return (
    <div className="flex-grow flex flex-col items-center md:block bg-white h-[100vh] overflow-x-scroll sticky left-0 top-0 pt-5 scrollbar-none border-r">
      <h1 className="md:pl-5 md:hidden font-bold uppercase text-customPurple">BS</h1>
          ...
          <section className="md:pt-5 w-full" key={index}>
            ...
            <ul className="text-customPurple">
              {section.pages.map((page, index) => (
                <li
                  title = {page.title}
                  className="md:pl-5 py-2 cursor-pointer space-x-2 md:hover:bg-gradient-to-tr from-transparent to-customPurple w-full hover:text-indigo-700 md:hover:text-inherit"
                  key={index}
                >
                  {page.icon}
      ...
      <div title="Logout" className="sticky bottom-0 py-5 shadow-[0px_-7px_10px_-4px_rgba(0,0,0,0.1)] bg-white text-customPurple flex items-center justify-center uppercase font-semibold hover:text-indigo-700 cursor-pointer">
        <span className="text-customGray mr-3 hidden md:inline">Logout</span> <LogoutRoundedIcon/>
      </div>
    </div>
  );
}
